...
- Sign inversion: changing the interaction effect from inhibition to activation and vice-versa (**Sign inversion**)
- Combining all the above (**all**)

Note that each type of scrambling results produces a scrambled topology with the same input and output degree distribution and as such, the scale-free property of the CASCADE 1.0 topology remains unchanged.
...
```{r zero-ss-topologies-fig, fig.width=7, fig.height=5, dpi=300, cache=TRUE, fig.cap='Percentage of topologies that did not have any boolean model with a stable state after simulation with Gitsbe ended. Every possible topology scrambling type is represented.', fig.align='center'}
...
```{r eff-scrambling-figs, message=FALSE, fig.width=7, fig.height=5, dpi=300, cache=TRUE, fig.show='hold', out.width='50%', fig.cap='Effect scrambling vs Performance (ROC and PR AUC)', fig.align='center'}
ggpubr::ggscatter(data = scrambled_topo_res %>% 
...
```{r all-scrambling-figs, message=FALSE, fig.width=7, fig.height=5, dpi=300, cache=TRUE, fig.show='hold', out.width='50%', fig.cap='Source, Target node and effect scrambling vs Performance (ROC and PR AUC)', fig.align='center'}
ggpubr::ggscatter(data = scrambled_topo_res %>% 
...
```{r src-scrambling-figs-2, warning=FALSE, fig.width=7, fig.height=5, dpi=300, cache=TRUE, fig.show='hold', out.width='50%', fig.cap='Source scrambling + bootstrap CASCADE 1.0 model results vs Performance (ROC and PR AUC)', fig.align='center'}
# ROC results
scrambled_topo_res %>%
  filter(scramble_type == 'source' | scramble_type == 'none', !is.na(roc_auc)) %>%
...
```{r trg-scrambling-figs-2, warning=FALSE, fig.width=7, fig.height=5, dpi=300, cache=TRUE, fig.show='hold', out.width='50%', fig.cap='Target node scrambling + bootstrap CASCADE 1.0 model results vs Performance (ROC and PR AUC)', fig.align='center'}
# ROC results
scrambled_topo_res %>%
  filter(scramble_type == 'target' | scramble_type == 'none', !is.na(roc_auc)) %>%
  ggplot(aes(x = grp, y = roc_auc, fill = grp)) +
...
```{r eff-scrambling-figs-2, warning=FALSE, fig.width=7, fig.height=5, dpi=300, cache=TRUE, fig.show='hold', out.width='50%', fig.cap='Effect scrambling + bootstrap CASCADE 1.0 model results vs Performance (ROC and PR AUC)', fig.align='center'}
# ROC results
scrambled_topo_res %>%
  filter(scramble_type == 'effect' | scramble_type == 'none', !is.na(roc_auc)) %>%
  ggplot(aes(x = grp, y = roc_auc, fill = grp)) +
...
```{r all-scrambling-figs-2, warning=FALSE, fig.width=7, fig.height=5, dpi=300, cache=TRUE, fig.show='hold', out.width='50%', fig.cap='Source, Target node and effect scrambling + bootstrap CASCADE 1.0 model results vs Performance (ROC and PR AUC)', fig.align='center'}
# ROC results
scrambled_topo_res %>%
  filter(scramble_type == 'all' | scramble_type == 'none', !is.na(roc_auc)) %>%
  ggplot(aes(x = grp, y = roc_auc, fill = grp)) +
...
model_predictions = emba::get_model_predictions("results/link-only/cascade_2.0_ss_150sim_fixpoints_bliss_model_predictions.tab")
...
```{r load-data-fit-vs-auc-topo, eval=FALSE}
# get flipped training data results
