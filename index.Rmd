...
```{r fit-evolution, dpi=300, fig.align='center', cache=TRUE, fig.cap='Fitness Evolution (10 simulations, CASCADE 1.0)'}
...
```{r fit-evolution-3, dpi=300, fig.align='center', cache=TRUE, warning=FALSE, fig.cap='Fitness Evolution (150 simulations, link operator mutations, CASCADE 2.0)'}
...
:::{.note}
We run `Gitsbe` simulations with $50$ topology mutations (bootstrap value, reduced to $10$ after models with stable states have been found), both for $50$ and $150$ simulations and both **fitting to steady state** (calibrated models) and to a **proliferative phenotype** (so not random models but as close as it can get to that since we are testing topology mutations).
...
You can of course change several other parameters in the input files or the script itself (e.g. number of simulations to run).
