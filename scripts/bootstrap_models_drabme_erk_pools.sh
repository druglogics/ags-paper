...
# all other options to default values in v1.2.1
...
do
  ...
  # run Drabme
  start=`date +%s`
  java -cp ../target/synergy-1.2.1-jar-with-dependencies.jar eu.druglogics.drabme.Launcher --project=cascade_2.0_${models_dir}_bliss_batch_${batch} --modelsDir=${sample_models_dir} --drugs=drugpanel --perturbations=perturbations --config=config --modeloutputs=modeloutputs > /dev/null 2>&1
