...
# The `scrambled_topologies` directory is produced via the `gen_scrambled_topologies.R`
# script and should also be placed in the repository root

training_files=`ls scrambled_topologies`
files_num=`ls scrambled_topologies | wc -l`
count=0

# change drabme config option (use bliss synergy assessement)
# all other option to default values in v1.2.0
...
for file in ${training_files}; do
  ...
  cat scrambled_topologies/$file > ags_cascade_1.0/network.sif
