...
start=`date +%s`
java -cp ../target/synergy-1.2.1-jar-with-dependencies.jar eu.druglogics.gitsbe.Launcher --project=cascade_2.0_rand_1000sim --network=network.sif --trainingdata=training --config=config --drugs=drugpanel --modeloutputs=modeloutputs > /dev/null 2>&1
