...
ggplot2::ggsave(filename = 'scripts/figures/figure_S7.pdf', width = 7, height = 5, device = cairo_pdf)
...
pr_fig = ggboxplot(res, x = "param", y = "pr_auc", fill = "param", palette = "Set1",
  ...
  theme(legend.position = "none")
ggplot2::ggsave(filename = 'scripts/figures/figure_9.pdf', width = 7, height = 5, device = cairo_pdf)

# Both plots in one
ggpubr::ggarrange(roc_fig, pr_fig, labels = LETTERS[1:2], nrow = 1, ncol = 2)
ggplot2::ggsave(filename = 'scripts/figures/figure_9_ROC_and_PR.pdf', width = 14, height = 5, device = cairo_pdf)
