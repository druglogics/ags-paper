...
# link-operator mutations only: `1.2.0` version has already that configuration
...
start=`date +%s`
java -cp ../target/synergy-1.2.0-jar-with-dependencies.jar eu.druglogics.gitsbe.Launcher --project=gitsbe_link_only_cascade_2.0_ss --network=network.sif --trainingdata=training --config=config --drugs=drugpanel --modeloutputs=modeloutputs > /dev/null 2>&1
...
start=`date +%s`
java -cp ../target/synergy-1.2.0-jar-with-dependencies.jar eu.druglogics.gitsbe.Launcher --project=gitsbe_topology_only_cascade_2.0_ss --network=network.sif --trainingdata=training --config=config --drugs=drugpanel --modeloutputs=modeloutputs > /dev/null 2>&1
runtime=$(($(date +%s)-$start))
echo -e Execution Time: "$(($runtime / 60)) minutes\n"
...
start=`date +%s`
java -cp ../target/synergy-1.2.0-jar-with-dependencies.jar eu.druglogics.gitsbe.Launcher --project=gitsbe_topo_and_link_cascade_2.0_ss --network=network.sif --trainingdata=training --config=config --drugs=drugpanel --modeloutputs=modeloutputs > /dev/null 2>&1
runtime=$(($(date +%s)-$start))
echo -e Execution Time: "$(($runtime / 60)) minutes\n"
