...
# file `scrambled_topologies_cascade_2.0.tar.gz`
...
for (exp_sim in exp_sims) {
  for(i in 1:10) {
    ...
    readr::write_tsv(x = edge_tbl %>% mutate(source = permut_sources),
      file = paste0(output_dir, "/cascade2_src_permut_", count, ".sif"),
      col_names = FALSE)
    # targets scrambled
    readr::write_tsv(x = edge_tbl %>% mutate(target = permut_targets),
      file = paste0(output_dir, "/cascade2_trg_permut_", count, ".sif"),
      col_names = FALSE)
    # effects scrambled
    readr::write_tsv(x = edge_tbl %>% mutate(effect = permut_effects),
      file = paste0(output_dir, "/cascade2_eff_permut_", count, ".sif"),
      col_names = FALSE)
    # all 3 columns scrambled!
    readr::write_tsv(x = edge_tbl %>% mutate(source = permut_sources,
      effect = permut_effects, target = permut_targets),
      file = paste0(output_dir, "/cascade2_all_permut_", count, ".sif"),
      col_names = FALSE)
    count = count + 1
  }
}
