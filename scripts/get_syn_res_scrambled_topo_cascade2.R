...
# directory with the scrambled CASCADE 2.0 topology files
# see Zenodo dataset http://tiny.cc/ags-paper-zenodo, file `scrambled_topologies_cascade_2.0.tar.gz`
topologies_dir = '/home/john/tmp/ags-paper/scrambled_topologies_cascade2'

# directory with the results of the script 'run_druglogics_synergy_scrambled_topo_cascade2.sh'
# See Zenodo dataset http://tiny.cc/ags-paper-zenodo, file `synergy-res-scrambled-topo-cascade2.tar.gz`
syn_res_dir = '/home/john/tmp/ags-paper/synergy-res-scrambled-topo-cascade2'

# CASCADE 2.0 topology
...
# observed synergies from (Flobak et al. 2019)
obs_syn_file = 'data/observed_synergies_cascade_2.0'
...
for (topo_file in list.files(path = topologies_dir, full.names = TRUE)) {
  ...
  topo_pat = stringr::str_extract(string = topo_file, pattern = "(cascade2_.*.sif)")
...
scrambled_topo_res_cascade2 = dplyr::bind_rows(data_list)
saveRDS(object = scrambled_topo_res_cascade2, file = 'data/scrambled_topo_res_cascade2.rds')
